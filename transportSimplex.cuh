...
public:
	...
	TsSignature(int nin, TF *fin, float * win):n(nin), features(fin), weights(win){};
};
...
typedef struct TsFlow {
	...
	float amount;							// Amount of flow from signature1.features[from] to signature2.features[to]
} TsFlow;
...
private:
float ** _tsC;				// Cost matrix
float _tsMaxC;						// Maximum of all costs
float _tsMaxW;						// Maximum of all weights
...
#define PROCEED_IF_VALID_ADDRESS(pointer, variableName)  isValidMallocAddress((void*)pointer, variableName, __FILE__, __LINE__)

CUDA_CALLABLE_MEMBER
bool isValidMallocAddress(void* pointer, char* variableName, const char *file, const int line)
{
    bool isValid = true;
    if(pointer == NULL)
    {
        printf("Malloc failed at %s:%i : %s\n", file, line, variableName);
        isValid = false;
    }
    return isValid;
}
...
CUDA_CALLABLE_MEMBER
void setGroundDistance(float** iGroundDistance, int numberOfBins)
{
    groundDistance = new float*[numberOfBins];
    PROCEED_IF_VALID_ADDRESS(groundDistance, "groundDistance");
    
	//printf("inside setGroundDistance \n");
	for(int i = 0; i < numberOfBins; i++)
	{
        groundDistance[i] = new float[numberOfBins];
        PROCEED_IF_VALID_ADDRESS(groundDistance[i], "groundDistance[i]");
        
		for(int j = 0; j < numberOfBins; j++)
		{
			//printf("%f   ", iGroundDistance[i][j]);
			groundDistance[i][j] = iGroundDistance[i][j];
		}
		//printf("\n");
	}
}
...
template <class TF>
CUDA_CALLABLE_MEMBER
float transportSimplex(TsSignature<TF> *signature1, TsSignature<TF> *signature2, float (*grndDist)(TF *, TF *, float** groundDistance), int patchwidth, 
	...
	float * src = NULL;						//Array of source supplies
	float * snk =NULL;							//Array of sink demands
    ...
    //printf("sink and source are collected");
	...
	//try {
        ...
        PROCEED_IF_VALID_ADDRESS(basics, "basics");
	
		isBasic = new bool*[n1];
        PROCEED_IF_VALID_ADDRESS(isBasic, "isBasic");
		for(i = 0; i < n1; i++) 
			isBasic[i] = NULL;

		for(i = 0; i < n1; i++) {
			isBasic[i] = new bool[n2];
            PROCEED_IF_VALID_ADDRESS(isBasic[i], "isBasic[i]");
			for (j=0; j < n2; j++)
				isBasic[i][j] = 0;
		}

		srcBasics = new TsBasic*[n1];
        PROCEED_IF_VALID_ADDRESS(srcBasics, "srcBasics");
		for (i = 0; i < n1; i++)
			srcBasics[i] = NULL;
	
		snkBasics = new TsBasic*[n2];
        PROCEED_IF_VALID_ADDRESS(srcBasics, "snkBasics");
		for (i = 0; i < n2; i++)
			snkBasics[i] = NULL;
		...
		_tsMaxC = 0;
		_tsC = new float*[n1];
        PROCEED_IF_VALID_ADDRESS(_tsC, "_tsC*");
		for(i=0; i < n1; i++) 
			_tsC[i] = NULL;

		for(i=0, P1=signature1->features; i < n1; i++, P1++) {
			_tsC[i] = new float[n2];					//What happens if bad  here?
            PROCEED_IF_VALID_ADDRESS(_tsC[i], "_tsC[i]");
			for(j=0, P2=signature2->features; j < n2; j++, P2++) {
				if (i == signature1->n || j == signature2->n) {
					_tsC[i][j] = 0;			// cost is zero for flow to or from a dummy
	         		...
	         		_tsC[i][j] = grndDist(P1, P2, groundDistance);
					if (_tsC[i][j] < 0) return; //throw TsErrBadInput;
				}
		...
		src = new float[n1];					//init the source array
        PROCEED_IF_VALID_ADDRESS(src, "src");
		for (i = 0; i < signature1->n; i++) {
			src[i] = signature1->weights[i];
			if (src[i] < 0) return; //throw TsErrBadInput;
		}
		if (n1 != signature1->n)
			src[signature1->n] = -diff;
	
		snk = new float[n2];					//init the sink array
        PROCEED_IF_VALID_ADDRESS(snk, "snk");
		for (i = 0; i < signature2->n; i++) {
			snk[i] = signature2->weights[i];
			if (snk[i] < 0) return; //throw TsErrBadInput;
		}
		if (n2 != signature2->n)
		...
		//printf("memory allocations are done\n");
    ...
    for(int i = 0; i < patchwidth; i++)
        delete[] groundDistance[i];
    
    delete[] groundDistance;

   // printf("memory allocation deleted\n");
...
*/
CUDA_CALLABLE_MEMBER
float _pivot(TsBasic * basics, TsBasic ** srcBasics, TsBasic ** snkBasics, bool ** isBasic, int n1, int n2)  {
	...
	int i,j, lowI, lowJ;
	float objectiveValue = TSINFINITY, oldObjectiveValue = 0;
	float lowVal;
	int numPivots = 0;

	//try {
		srcDuals = new float[n1];
        PROCEED_IF_VALID_ADDRESS(srcDuals, "srcDuals");
		snkDuals = new float[n2];
        PROCEED_IF_VALID_ADDRESS(snkDuals, "snkDuals");
        
		stonePath = new TsStone[n1 + n2];
        PROCEED_IF_VALID_ADDRESS(stonePath, "stonePath");
...
CUDA_CALLABLE_MEMBER
inline void addPenalty(TsVogPen * pitr, float cost, int i) {
...
**********************/
CUDA_CALLABLE_MEMBER
void _initVogel(float *S, float *D, TsBasic * basicsEnd, TsBasic ** srcBasics, TsBasic ** snkBasics, bool ** isBasic, int n1, int n2) {
	int i, j;
	...
	//try {
        ...
        PROCEED_IF_VALID_ADDRESS(srcPens, "srcPens");
		snkPens = new TsVogPen[n2];
        PROCEED_IF_VALID_ADDRESS(snkPens, "snkPens");
	/*} catch (std::bad_alloc) {
		delete[] srcPens;
		delete[] snkPens;
		throw;	
...
**********************/
CUDA_CALLABLE_MEMBER
void _initRussel(float *S, float *D, TsBasic * basicsEnd, TsBasic ** srcBasics, TsBasic ** snkBasics, bool ** isBasic, int n1, int n2) {
	float ** Delta = NULL;
	int i, j, lowI, lowJ;
	TsRusPen *U  = NULL;
	TsRusPen *V = NULL;
	...
	//try {
		Delta = new float*[n1];
        PROCEED_IF_VALID_ADDRESS(Delta, "Delta");
		for (i = 0; i < n1; i++)
        {
			Delta[i] = new float[n2];
            PROCEED_IF_VALID_ADDRESS(Delta[i], "Delta[i]");
        }

		U = new TsRusPen[n1];
        PROCEED_IF_VALID_ADDRESS(U, "U");
		V = new TsRusPen[n2];
        PROCEED_IF_VALID_ADDRESS(V, "V");
